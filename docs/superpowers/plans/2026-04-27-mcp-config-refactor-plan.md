...
**Goal:** 在维持 MCP “暂不纳入主线重构目标”这一共识不变的前提下，只做 MCP 域内的小步整理：固化 create/update 的当前行为，提取极小的本地 helper，避免这条链路继续靠手写重复逻辑生长。
...
- MCP 计划里的现有 seam 优先级如下：
  ...
  - Task 3：优先继续测现有 `createResourceHandler(...)`，验证 draft 组装提取前后一致
