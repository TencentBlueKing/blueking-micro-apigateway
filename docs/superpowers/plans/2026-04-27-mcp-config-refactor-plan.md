...
- Task 0 至少覆盖 4 类现状：create 会把 outer `name` 写入 `config`；consumer create 写的是 `username`；update 在提供 `name` 时会同步更新 `config`；不提供 `name` 时保留原有 config 形态。
...
至少覆盖下面 4 类现状，断言都走真实 create/update handler，而不是新 helper：
...
在 `mcp_resource_crud_helpers.go` 里新增：
...
func TestPrepareMCPUpdateConfig(t *testing.T) {
    ...
    t.Run("inject name when provided", func(t *testing.T) {
    ...
    t.Run("keep config untouched when name is empty", func(t *testing.T) {
        config, err := prepareMCPUpdateConfig(
            constant.Route,
            map[string]any{"uri": "/demo"},
            "",
...
在 `mcp_resource_crud_helpers.go` 中补充：
