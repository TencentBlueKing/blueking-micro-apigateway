...
├── pkg/utils/schema/
├── pkg/resourcecodec/                 # new package planned for request-draft/payload-building logic
└── tests/
    ├── integration/
    └── util/
```

**Structure Decision**: Keep the existing single-service backend structure. Introduce a new `pkg/resourcecodec/` package for identity resolution, resource-draft preparation, and APISIX payload building, while keeping request handlers, serializers, middleware, biz orchestration, publisher code, and model hooks as thin adapters around that shared layer. Add regression tests close to current entrypoints in their existing package-local test files.
...
### Phase 1 - Introduce Resource Draft And Payload Building

1. Add `pkg/resourcecodec/` with per-resource adapters behind shared interfaces:
   - `ResolveIdentity(...)`
   - `PrepareDraft(...)`
   - `BuildPayload(..., profile)`
   - legacy-tolerant helpers for extracting config specs from existing stored rows when needed
2. Define one shared resource-draft representation used internally for:
...
1. Migrate publish assembly in `pkg/biz/publish.go` to use `resourcecodec.BuildPayload(..., ETCD)` rather than per-resource JSON surgery.
2. Migrate request/import validation paths to use resolved identity + prepared draft + `BuildPayload(..., DATABASE)` before persistence.
3. Preserve `EtcdPublisher.Validate` as the final publish gate, but ensure it consumes the same built payload family used earlier in the flow.
