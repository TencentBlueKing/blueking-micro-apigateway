...
func PluginConfigCreate(c *gin.Context) {
	...
	// plugin_config schema requires config.id, but that ID is generated by the server. Bind first so validation uses
	// the same ID that will be written to the model config.
	req.ID = idx.GenResourceID(constant.PluginConfig)
