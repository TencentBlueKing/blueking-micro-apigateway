...
func getConsumerGroupQuery(ctx context.Context) repo.IConsumerGroupDo {
	return repo.ConsumerGroup.WithContext(ctx).Where(field.Attrs(map[string]interface{}{
		"gateway_id": ginx.GetGatewayInfoFromContext(ctx).ID,
	}))
}

// getConsumerGroupQueryWithTx 获取 ConsumerGroup 查询对象
func getConsumerGroupQueryWithTx(ctx context.Context, tx *repo.Query) repo.IConsumerGroupDo {
...
func ListConsumerGroups(ctx context.Context) ([]*model.ConsumerGroup, error) {
	...
	return getConsumerGroupQuery(ctx).Order(u.UpdatedAt.Desc()).Find()
}
...
func ListPagedConsumerGroups(
	...
	u := repo.ConsumerGroup
	query := getConsumerGroupQuery(ctx)
...
func BatchCreateConsumerGroups(ctx context.Context, consumerGroups []*model.ConsumerGroup) error {
	if ginx.GetTx(ctx) != nil {
		return getConsumerGroupQueryWithTx(ctx, ginx.GetTx(ctx)).Create(consumerGroups...)
	}
...
func UpdateConsumerGroup(ctx context.Context, consumerGroup model.ConsumerGroup) error {
	u := repo.ConsumerGroup
	_, err := getConsumerGroupQuery(ctx).Where(u.ID.Eq(consumerGroup.ID)).Select(
		u.Name,
		u.Config,
		u.Status,
		u.Updater,
...
func GetConsumerGroup(ctx context.Context, id string) (*model.ConsumerGroup, error) {
	u := repo.ConsumerGroup
	return getConsumerGroupQuery(ctx).Where(u.ID.Eq(id)).First()
}
...
func QueryConsumerGroups(ctx context.Context, param map[string]interface{}) ([]*model.ConsumerGroup, error) {
	return getConsumerGroupQuery(ctx).Where(field.Attrs(param)).Find()
}

// ExistsConsumerGroup 查询 ConsumerGroup 是否存在
func ExistsConsumerGroup(ctx context.Context, id string) bool {
	u := repo.ConsumerGroup
	groups, err := getConsumerGroupQuery(ctx).Where(u.ID.Eq(id)).Find()
	if err != nil {
		return false
	}
	if len(groups) == 0 {
...
func BatchDeleteConsumerGroups(ctx context.Context, ids []string) error {
	...
	err := repo.Q.Transaction(func(tx *repo.Query) error {
		...
		_, err = getConsumerGroupQueryWithTx(ctx, tx).Where(u.ID.In(ids...)).Delete()
		return err
	})
	return err
}
...
func BatchRevertConsumerGroups(ctx context.Context, syncDataList []*model.GatewaySyncData) error {
	...
	err = repo.Q.Transaction(func(tx *repo.Query) error {
		...
		for _, consumerGroup := range consumerGroups {
			_, err := getConsumerGroupQueryWithTx(ctx, tx).Updates(consumerGroup)
			if err != nil {
				return err
			}
		}
