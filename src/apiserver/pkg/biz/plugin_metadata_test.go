...
func TestBuildPluginMetadataQuery(t *testing.T) {
		...
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectPanic {
				assert.Panics(t, func() {
					buildPluginMetadataQuery(tt.ctx)
				}, "没有网关信息时应该 panic")
				return
			}

			query := buildPluginMetadataQuery(tt.ctx)
...
func TestBuildPluginMetadataQueryWithTx(t *testing.T) {
	gateway := data.Gateway1WithBkAPISIX()
	ctx := ginx.SetGatewayInfoToContext(context.Background(), gateway)
	tx := repo.Q
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectPanic {
				assert.Panics(t, func() {
					buildPluginMetadataQueryWithTx(tt.ctx, tt.tx)
				}, "没有网关信息时应该 panic")
				return
			}

			query := buildPluginMetadataQueryWithTx(tt.ctx, tt.tx)
