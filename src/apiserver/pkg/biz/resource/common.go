...
var resourceTableMap = map[constant.APISIXResource]string{
	...
	constant.Schema:         model.GatewayCustomPluginSchema{}.TableName(),
}
...
// buildCommonDbQuery builds a plain gorm query filtered by gateway_id.
...
// ResourceTableName returns the DB table name for the given resource type.
func ResourceTableName(resourceType constant.APISIXResource) string {
	return resourceTableMap[resourceType]
}

// NewTypedResourceModel converts a common resource model into its typed DB model.
func NewTypedResourceModel(resourceType constant.APISIXResource, resource model.ResourceCommonModel) (any, error) {
	resourceModel, exists := resourceModelMap[resourceType]
	if !exists {
		return nil, fmt.Errorf("resource type: %s is invalid", resourceType)
	}
...
func GetSchemaByIDs(
	...
	query := buildCommonDbQuery(ctx, constant.Schema)
	if len(ids) <= constant.DBConditionIDMaxLength {
	...
	for i := 0; i < len(ids); i += constant.DBConditionIDMaxLength {
		...
		err := query.Where("auto_id IN ?", batchIDs).Find(&batchRes).Error
		if err != nil {
			return nil, err
		}
		res = append(res, batchRes...)
