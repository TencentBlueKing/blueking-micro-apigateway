...
func GetSchemaExprList(orderBy string) []field.Expr {
	...
	orderByExprList := utils.ParseOrderByExprList(ascFieldMap, descFieldMap, orderBy)
...
func ListPagedSchema(
	...
	return query.Order(orderByExprs...).FindByPage(page.Offset, page.Limit)
}
...
func BatchCreateSchema(ctx context.Context, schemas []*model.GatewayCustomPluginSchema) error {
	if ginx.GetTx(ctx) != nil {
		return buildSchemaQueryWithTx(ctx, ginx.GetTx(ctx)).CreateInBatches(schemas, constant.DBBatchCreateSize)
	}
...
func GetSchemaByName(ctx context.Context, name string) (*model.GatewayCustomPluginSchema, error) {
	u := repo.GatewayCustomPluginSchema
	schemaInfo, err := buildSchemaQuery(ctx).Where(u.Name.Eq(name)).First()
...
func BatchGetSchemaByName(ctx context.Context, names []string) ([]*model.GatewayCustomPluginSchema, error) {
	u := repo.GatewayCustomPluginSchema
	schemaInfo, err := buildSchemaQuery(ctx).Where(u.Name.In(names...)).Find()
	return schemaInfo, err
}
...
func GetSchemaByID(ctx context.Context, id int) (*model.GatewayCustomPluginSchema, error) {
	u := repo.GatewayCustomPluginSchema
	schemaInfo, err := buildSchemaQuery(ctx).Where(u.AutoID.Eq(id)).First()
	return schemaInfo, err
}
...
func DuplicatedSchemaName(ctx context.Context, id int, name string) bool {
	u := repo.GatewayCustomPluginSchema
	query := buildSchemaQuery(ctx).Where(u.Name.Eq(name))
	if id != 0 {
...
func GetCustomizePluginExampleList(ctx context.Context) ([]*schemax.Plugin, error) {
	var plugins []*schemax.Plugin
	schemaList, err := ListSchema(ctx)
	if err != nil {
		return nil, err
	}
	for _, s := range schemaList {
		var plugin schemax.Plugin
