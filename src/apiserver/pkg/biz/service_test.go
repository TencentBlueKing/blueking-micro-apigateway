		...
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectPanic {
				assert.Panics(t, func() {
					buildServiceQuery(tt.ctx)
				}, "没有网关信息时应该 panic")
				return
			}

			query := buildServiceQuery(tt.ctx)
			assert.NotNil(t, query, "查询对象不应该为空")
			assert.Implements(t, (*repo.IServiceDo)(nil), query, "应该实现 IServiceDo 接口")
		})
	}
}

func TestBuildServiceQueryWithTx(t *testing.T) {
	gateway := data.Gateway1WithBkAPISIX()
	ctx := ginx.SetGatewayInfoToContext(context.Background(), gateway)
	tx := repo.Q
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectPanic {
				assert.Panics(t, func() {
					buildServiceQueryWithTx(tt.ctx, tt.tx)
				}, "没有网关信息时应该 panic")
				return
			}

			query := buildServiceQueryWithTx(tt.ctx, tt.tx)
