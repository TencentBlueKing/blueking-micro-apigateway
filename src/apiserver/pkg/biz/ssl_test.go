...
func TestBuildSSLQuery(t *testing.T) {
		...
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectPanic {
				assert.Panics(t, func() {
					buildSSLQuery(tt.ctx)
				}, "没有网关信息时应该 panic")
				return
			}

			query := buildSSLQuery(tt.ctx)
			assert.NotNil(t, query, "查询对象不应该为空")
			assert.Implements(t, (*repo.ISSLDo)(nil), query, "应该实现 ISSLDo 接口")
		})
	}
}

func TestBuildSSLQueryWithTx(t *testing.T) {
	gateway := data.Gateway1WithBkAPISIX()
	ctx := ginx.SetGatewayInfoToContext(context.Background(), gateway)
	tx := repo.Q
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectPanic {
				assert.Panics(t, func() {
					buildSSLQueryWithTx(tt.ctx, tt.tx)
				}, "没有网关信息时应该 panic")
				return
			}

			query := buildSSLQueryWithTx(tt.ctx, tt.tx)
