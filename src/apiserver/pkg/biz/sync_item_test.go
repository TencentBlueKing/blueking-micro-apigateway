...
func TestBuildSyncedItemQuery(t *testing.T) {
		...
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectPanic {
				assert.Panics(t, func() {
					buildSyncedItemQuery(tt.ctx)
				}, "没有网关信息时应该 panic")
				return
			}

			query := buildSyncedItemQuery(tt.ctx)
			assert.NotNil(t, query, "查询对象不应该为空")
			assert.Implements(t, (*repo.IGatewaySyncDataDo)(nil), query, "应该实现 IGatewaySyncDataDo 接口")
		})
	}
}
