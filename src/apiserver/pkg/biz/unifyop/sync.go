...
// Package unifyop contains sync/import/revert orchestration for APISIX resources.
package unifyop
...
func SyncAll(ctx context.Context) {
	gateways, err := gatewaybiz.ListGateways(ctx, 0)
	if err != nil {
...
func removeDuplicatedResource(
	...
	// 查询数据库所有的资源
	resourceList, err := resourcebiz.BatchGetResources(ctx, resourceType, []string{})
	if err != nil {
...
func AddSyncedResources( //nolint:gocyclo
	...
	items, err := syncdatabiz.QuerySyncedItems(ctx, queryParam)
	if err != nil {
		return nil, err
	}
	if len(items) == 0 {
...
func insertSyncedResourcesModel(
	...
	for resourceType, itemList := range typeSyncedItemMap {
		...
		switch resourceType {
		case constant.Route:
			err = resourcebiz.BatchCreateRoutes(ctx, resourceList.([]*model.Route)) //nolint:forcetypeassert
			if err != nil {
				return err
			}
		case constant.Service:
			services := resourceList.([]*model.Service) //nolint:forcetypeassert
			err = resourcebiz.BatchCreateServices(ctx, services)
			if err != nil {
				return err
			}
		case constant.Upstream:
			upstreams := resourceList.([]*model.Upstream) //nolint:forcetypeassert
			err = resourcebiz.BatchCreateUpstreams(ctx, upstreams)
			if err != nil {
				return err
			}
		case constant.PluginConfig:
			//nolint:forcetypeassert
			err = resourcebiz.BatchCreatePluginConfigs(ctx, resourceList.([]*model.PluginConfig))
			if err != nil {
				return err
			}
		case constant.PluginMetadata:
			//nolint:forcetypeassert
			err = resourcebiz.BatchCreatePluginMetadatas(ctx, resourceList.([]*model.PluginMetadata))
			if err != nil {
				return err
			}
		case constant.Consumer:
			consumers := resourceList.([]*model.Consumer) //nolint:forcetypeassert
			err = resourcebiz.BatchCreateConsumers(ctx, consumers)
			if err != nil {
				return err
			}
		case constant.ConsumerGroup:
			//nolint:forcetypeassert
			err = resourcebiz.BatchCreateConsumerGroups(ctx, resourceList.([]*model.ConsumerGroup))
			if err != nil {
				return err
			}
		case constant.GlobalRule:
			globalRules := resourceList.([]*model.GlobalRule) //nolint:forcetypeassert
			err = resourcebiz.BatchCreateGlobalRules(ctx, globalRules)
			if err != nil {
				return err
			}
		case constant.SSL:
			err = resourcebiz.BatchCreateSSL(ctx, resourceList.([]*model.SSL)) //nolint:forcetypeassert
			if err != nil {
				return err
			}
		case constant.Proto:
			err = resourcebiz.BatchCreateProtos(ctx, resourceList.([]*model.Proto)) //nolint:forcetypeassert
			if err != nil {
				return err
			}
		case constant.StreamRoute:
			streamRoutes := resourceList.([]*model.StreamRoute) //nolint:forcetypeassert
			err = resourcebiz.BatchCreateStreamRoutes(ctx, streamRoutes)
			if err != nil {
				return err
			}
		}
...
func UploadResources(
	...
	err = repo.Q.Transaction(func(tx *repo.Query) error {
		...
		for resourceType, itemList := range updateTypeResourcesTypeMap {
			...
			err = resourcebiz.DeleteResourceByIDs(ctx, resourceType, ids)
			if err != nil {
				return err
			}
		}
		...
		if len(updateSchemaNames) > 0 {
			existingSchemas, err := schemabiz.BatchGetSchemaByName(ctx, updateSchemaNames)
			if err != nil {
				return err
			}
			...
			err = schemabiz.BatchUpdateSchema(ctx, existingSchemas)
			if err != nil {
				return err
			}
		}
		...
		if len(schemaInfoList) > 0 {
			err = schemabiz.BatchCreateSchema(ctx, schemaInfoList)
			if err != nil {
				return err
			}
		}
...
func GetSyncItemsAssociatedResources(
	...
	addAssociatedID := func(resourceType constant.APISIXResource, id string) {
		...
		resourceKey := fmt.Sprintf(constant.ResourceKeyFormat, resourceType, id)
	...
	for _, item := range items {
		addAssociatedID(constant.Service, item.GetServiceID())
		addAssociatedID(constant.Upstream, item.GetUpstreamID())
		addAssociatedID(constant.PluginConfig, item.GetPluginConfigID())
...
func (s *UnifyOp) SyncWithPrefix(ctx context.Context, prefix string) (map[constant.APISIXResource]int, error) {
	...
	// 获取已同步资源
	syncedItems, err := syncdatabiz.QuerySyncedItems(ctx, map[string]any{})
	if err != nil {
		return nil, err
	}
...
var revertConfigByIDListFunc = map[constant.APISIXResource]func(ctx context.Context,
	...
	constant.Route:          resourcebiz.BatchRevertRoutes,
	constant.Service:        resourcebiz.BatchRevertServices,
	constant.Upstream:       resourcebiz.BatchRevertUpstreams,
	constant.PluginConfig:   resourcebiz.BatchRevertPluginConfigs,
	constant.PluginMetadata: resourcebiz.BatchRevertPluginMetadatas,
	constant.Consumer:       resourcebiz.BatchRevertConsumers,
	constant.ConsumerGroup:  resourcebiz.BatchRevertConsumerGroups,
	constant.GlobalRule:     resourcebiz.BatchRevertGlobalRules,
	constant.SSL:            resourcebiz.BatchRevertSSLs,
	constant.Proto:          resourcebiz.BatchRevertProtos,
	constant.StreamRoute:    resourcebiz.BatchRevertStreamRoutes,
}
...
func (s *UnifyOp) RevertConfigByIDList(
	...
	// 状态机判断
	resources, err := resourcebiz.BatchGetResources(ctx, resourceType, idList)
	if err != nil {
		return err
	}
...
func GetResourceConfigDiffDetail(
	...
	syncedResource, err := syncdatabiz.GetSyncedItemByResourceTypeAndID(ctx, resourceType, id)
	...
	// 获取编辑区资源配置
	resourceInfo, err := resourcebiz.GetResourceByID(ctx, resourceType, id)
	if err != nil {
		return nil, err
	}
	// 删除状态需要特殊处理
