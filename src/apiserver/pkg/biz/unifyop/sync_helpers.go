...
func backfillStoredSnapshotFields(ctx context.Context, resources []*model.GatewaySyncData) error {
	...
	if len(globalRuleIDs) > 0 {
		globalRules, err := resourcebiz.QueryGlobalRules(ctx, map[string]any{
	...
	if len(pluginConfigIDs) > 0 {
		pluginConfigs, err := resourcebiz.QueryPluginConfigs(ctx, map[string]any{
			"gateway_id": gatewayID,
			"id":         pluginConfigIDs,
		})
		if err != nil {
	...
	if len(consumerGroupIDs) > 0 {
		consumerGroups, err := resourcebiz.QueryConsumerGroups(ctx, map[string]any{
			"gateway_id": gatewayID,
			"id":         consumerGroupIDs,
		})
		if err != nil {
	...
	// Proto name 需要特殊处理
	if len(protoIDs) > 0 {
		protos, err := resourcebiz.QueryProtos(ctx, map[string]any{
			"gateway_id": gatewayID,
			"id":         protoIDs,
		})
		if err != nil {
	...
	if len(streamRouteIDs) > 0 {
		streamRoutes, err := resourcebiz.QueryStreamRoutes(ctx, map[string]any{
			"id": streamRouteIDs,
		})
		if err != nil {
			return err
...
func reconcilePluginMetadataSyncIDs(ctx context.Context, resources []*model.GatewaySyncData) error {
	...
	metadatas, err := resourcebiz.QueryPluginMetadatas(ctx, map[string]any{
