...
func TestBackfillStoredSnapshotFields(t *testing.T) {
	...
	assert.NoError(t, resourcebiz.CreateConsumerGroup(ctx, *consumerGroup))

	proto := data.Proto1(gatewayInfo, constant.ResourceStatusSuccess)
	proto.Name = "proto-from-db-" + suffix
	assert.NoError(t, resourcebiz.CreateProto(ctx, *proto))
	...
	assert.NoError(t, resourcebiz.CreateStreamRoute(ctx, *streamRoute))

	globalRule := data.GlobalRule1(gatewayInfo, constant.ResourceStatusSuccess)
	globalRule.Name = "gr-from-db-" + suffix
	assert.NoError(t, resourcebiz.CreateGlobalRule(ctx, *globalRule))
...
func TestReconcilePluginMetadataSyncIDs(t *testing.T) {
	...
	assert.NoError(t, resourcebiz.CreatePluginMetadata(ctx, *existing))
