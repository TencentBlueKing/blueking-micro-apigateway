		...
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectPanic {
				assert.Panics(t, func() {
					buildUpstreamQuery(tt.ctx)
				}, "没有网关信息时应该 panic")
				return
			}

			query := buildUpstreamQuery(tt.ctx)
			assert.NotNil(t, query, "查询对象不应该为空")
			assert.Implements(t, (*repo.IUpstreamDo)(nil), query, "应该实现 IUpstreamDo 接口")
		})
	}
}

func TestBuildUpstreamQueryWithTx(t *testing.T) {
	gateway := data.Gateway1WithBkAPISIX()
	ctx := ginx.SetGatewayInfoToContext(context.Background(), gateway)
	tx := repo.Q
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectPanic {
				assert.Panics(t, func() {
					buildUpstreamQueryWithTx(tt.ctx, tt.tx)
				}, "没有网关信息时应该 panic")
				return
			}

			query := buildUpstreamQueryWithTx(tt.ctx, tt.tx)
