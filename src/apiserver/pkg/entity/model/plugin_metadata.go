...
// TableName 设置表名
func (PluginMetadata) TableName() string {
	return "plugin_metadata"
}

// BeforeCreate 创建前钩子
func (p *PluginMetadata) BeforeCreate(tx *gorm.DB) (err error) {
...
// BeforeUpdate 更新前钩子
func (p *PluginMetadata) BeforeUpdate(tx *gorm.DB) (err error) {
	if err := p.HandleConfig(); err != nil {
		return err
	}
	// 关联自定义插件
...
// BeforeDelete 删除前钩子
func (p *PluginMetadata) BeforeDelete(tx *gorm.DB) (err error) {
	if err := p.HandleConfig(); err != nil {
		return err
	}
	// 添加审计
...
// AddAuditLog 添加审计
func (p *PluginMetadata) AddAuditLog(tx *gorm.DB, operation constant.OperationType) (err error) {
	// 排除批量删除，更新的情况
	if p.ID == "" {
		return nil
	}
