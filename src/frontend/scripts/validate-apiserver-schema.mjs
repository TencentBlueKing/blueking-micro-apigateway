...
const schemaDirectory = fileURLToPath(new URL('../../apiserver/pkg/utils/schema/3.17/', import.meta.url));
...
const compileSchema = (source, plugin, scope, schema, count = true) => {
  try {
    ...
    failures.push({ source, plugin, scope, errors: error.message });
    return undefined;
  }
};
...
const validateExample = (source, plugin, scope, schemaPath, example) => {
  ...
  if (schema === undefined) {
    failures.push({ source, plugin, scope, errors: `schema not found: ${schemaPath}` });
    return;
  }
  ...
  if (!validate(example)) {
    failures.push({ source, plugin, scope, errors: ajv.errorsText(validate.errors, { separator: '; ' }) });
  }
};
...
if (failures.length > 0) {
  ...
  console.log(`Compiled ${compiledSchemaCount} schema nodes and validated ${validatedExampleCount} examples.`);
}
