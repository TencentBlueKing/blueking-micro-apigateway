...
<template>
  <div>
    ...
    >
      <bk-form-item
        :class="{ 'mb-0': uris?.length > 1 && index === uris.length - 1 }"
    ...
    <icon
      ...
      class="icon-btn" color="#979BA5" name="plus-circle-shape" size="18" @click="handleAddItem"
    />
    <div :class="{ 'mt--24': uris?.length === 1 }" class="common-form-tips form-tips">
      <slot name="tooltips">{{ t('HTTP 请求路径，如 /foo/index.html，支持请求路径前缀 /foo/*。/* 代表所有路径') }}</slot>
    </div>
  </div>
</template>
