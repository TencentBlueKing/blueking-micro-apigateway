...
<template>
  <div>
    <list-content-article>
      ...
      <list-content-row :label="t('名称')">{{ pluginConfigs.name }}</list-content-row>
      <list-content-row :label="t('ID')">{{ pluginConfigs.id }}</list-content-row>
      <list-content-row v-if="pluginConfigs.config.desc" :label="t('描述')">{{
        pluginConfigs.config.desc
      }}
      </list-content-row>
      <list-content-row
        v-if="pluginConfigs.config.labels && Object.keys(pluginConfigs.config.labels).length" :label="t('标签')"
      >
        <tag-label :labels="pluginConfigs.config.labels" />
      </list-content-row>
      <list-content-rows-common :resource="pluginConfigs" />
    </list-content-article>

    <list-content-article v-if="pluginConfigs.config.plugins && Object.keys(pluginConfigs.config.plugins).length">
...
const { resource: pluginConfigs } = defineProps<IProps>();
