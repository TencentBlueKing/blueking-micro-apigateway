...
<template>
  <div>
    <list-content-article>
      ...
      <list-content-row :label="t('插件名称')">{{ pluginMetadata.name }}</list-content-row>
      <list-content-row :label="t('ID')">{{ pluginMetadata.id }}</list-content-row>
      <list-content-rows-common :resource="pluginMetadata" />
      <list-content-row label="Config">{{ pluginMetadata.config }}</list-content-row>
    </list-content-article>
  </div>
</template>
...
interface IProps {
  resource: IPluginMetadataDto
}

const { resource: pluginMetadata } = defineProps<IProps>();
